...
function qrbc_today_history_qr_bonus()
{
    if (current_user_can('manage_options')) {
        ...
        $bonuses = $wpdb->get_results($wpdb->remove_placeholder_escape($wpdb->prepare("SELECT * FROM $table_name WHERE created_at LIKE %s ORDER BY id DESC", "%" . $wpdb->esc_like($date) . "%")));
